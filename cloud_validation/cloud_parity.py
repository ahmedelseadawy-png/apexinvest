...
Part B: live-feed endpoints, baseline server (8002) vs cloud-project server (8001): identical JSON expected.
"""
...
M = os.environ.get("CLOUD_URL", "http://127.0.0.1:8010"); BASE_SRV = os.environ.get("LOCAL_URL", "http://127.0.0.1:8002")
KEY = {"X-Apex-Key": os.environ.get("CLOUD_KEY", "Cloud-Test-Key-0123456789")}   # cloud runs with the access gate ON
BASEDIR = os.environ.get("APEX_BASELINE", "/home/claude/baseline/apexinvest_backend")   # git archive original-desktop-baseline
OBJS = ["swing", "long_term", "day", "income", "analyze"]
os.makedirs("data/parity", exist_ok=True)
DATA_DIR = os.path.abspath("../mobile_validation/data")
...
# extra special files
specials = [("EGX_COMI_newest_first", "../mobile_validation/data/EGX_COMI_newest_first.csv", True), ("EGX_COMI, 1D_tv", "../mobile_validation/data/EGX_COMI, 1D_tv.csv", False),
            ("EGX_RAYA, 1D", "../mobile_validation/data/EGX_RAYA, 1D.csv", False), ("short_20bars", "../mobile_validation/data/short_20bars.csv", False)]
...
def compare_case(test, cur, mob, kind):
    ...
    for g, f in GROUPS.items():
        ...
        diffs_total += len(d)
        rows.append({"test": test, "path": kind, "group": g, "local": short(a), "cloud": short(b), "diff": ("0 (identical)" if not d else f"{len(d)} diffs, max|Δ|={md:g}; e.g. {d[0][0]}: {short(d[0][1])} vs {short(d[0][2])}"), "status": st})
...
for c in manifest:
    ...
    with open(fn, "rb") as fh:
        r = s.post(M + "/v1/analyses/csv", headers=KEY, files={"file": (os.path.basename(fn), fh, "text/csv")}, data={"symbol": c["symbol"], "objective": c["objective"]})
...
# ---------------- Part B: HTTP feed endpoints, baseline server vs cloud-project server
def both(path, method="GET", **kw):
    ...
    if files:
        ...
        a = requests.request(method, BASE_SRV + path, files=raw, **kw); b = requests.request(method, M + path, files=raw, headers=KEY, **kw)
    else:
        a = requests.request(method, BASE_SRV + path, **kw); b = requests.request(method, M + path, headers=KEY, **kw)
    return a, b
...
for sym in SYMS:
    for o in OBJS:
        ...
        ja, jb = a.json(), b.json()
        compare_case(f"FEED {sym}|{o}", {k: v for k, v in ja.items() if k not in ("candles",)}, {k: v for k, v in jb.items() if k not in ("candles",)}, "feed baseline-server-vs-cloud-server")
        # also compare full JSON incl. data_source & candles
        d = list(walk(ja, jb)); rows.append({"test": f"FEED {sym}|{o}", "path": "full JSON", "group": "full_response", "local": f"{len(json.dumps(ja))} bytes", "cloud": f"{len(json.dumps(jb))} bytes", "diff": "0 (byte-identical)" if not d else f"{len(d)} diffs e.g. {d[0][0]}", "status": "PASS" if not d else "FAIL"})
        nB += 1
other = []
def cmp_http(name, path, method="GET", **kw):
    ...
    rows.append({"test": name, "path": "endpoint baseline-vs-cloud", "group": "full_response", "local": f"HTTP {a.status_code}", "cloud": f"HTTP {b.status_code}", "diff": "0 (identical)" if not d else f"{len(d)} diffs e.g. {d[0]}", "status": "PASS" if not d else "FAIL"})
...
cmp_http("UPLOAD summary", "/v1/uploads", "POST", files={"file": ("EGX_COMI.csv", open("../mobile_validation/data/EGX_COMI, 1D_tv.csv", "rb"), "text/csv")})
...
json.dump(rows, open("cloud_parity_rows.json", "w"))
with open("cloud_parity_detail.csv", "w", newline="") as fh:
    w = csv.DictWriter(fh, fieldnames=["test", "path", "group", "local", "cloud", "diff", "status"]); w.writeheader(); w.writerows(rows)
